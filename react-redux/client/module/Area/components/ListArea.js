...
const ListArea = React.createClass({       
    ...
    handleDelete(){
        ...
        if(choices != "none" && choices!= undefined){
            if(choices == "all"){
                for(var i=0;i<this.props.listarea.length;i++){            
                    newChoices.push(this.props.listarea[i].id);
                }
            }
            else{
                for(var i=0;i<choices.length;i++){            
                    newChoices.push(this.props.listarea[choices[i]].id);
                }
            }
        }      
        
        this.props.DeleteAreaBatch(newChoices);
    },
    ...
    //Edit Button
    handleEdit(id){
        this.context.router.push('/area/'+id);
    },
    //Add Button
    handleAdd(){
             ...
             <div style={{padding:"30px",marginTop:"20px"}}>
                ...
                <TableHeader displaySelectAll={true} adjustForCheckbox={true} enableSelectAll={true}>                  
                    <TableRow>
                        ...
                        <TableHeaderColumn tooltip="The Status">Color</TableHeaderColumn>
                ...
                <TableBody displayRowCheckbox={true} deselectOnClickaway={false} showRowHover={true} stripedRows={false}>
                    {this.props.listarea.map((row, index) => (
                    <TableRow key={index} >
                        <TableRowColumn>{index+1}</TableRowColumn>
                        <TableRowColumn>{row.nameArea}</TableRowColumn>
